...
class PreferenceHelper :NSObject{
    ...
    func setLanguage(_ index:String)
    {
    ...
    {
        return (ph.value(forKey: KEY_LANGUAGE) as? String) ?? "0"
    }
    
    
    
    func getApiLangugae() -> String {
        if (ph.value(forKey: KEY_LANGUAGE) as? String) ?? "0" == "0" {
            return "1" // english
        } else if (ph.value(forKey: KEY_LANGUAGE) as? String) ?? "0" == "1" {
            return "4" // arabic
        } else {
            return "1"
        }
